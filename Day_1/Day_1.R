### 30DayChartChallenge | May 2023 - Day 1| part-to-whole ###
## Day 1 - Data source is www.osbm.nc.gov

# install packages  
#install.packages("janitor")
install.packages(c("tidyverser", "showtext", "janitor", "MetBrewer"))
...
setwd("C:/Users/ncber/OneDrive/Documentos/Practice in R/30daysChallenge/Day_1")
...
df%>%
  ...
  geom_text(aes(x=ifelse(type %in% c("Gas", "Electric"), xmax+0.01, xmin+0.01), 
                y=ifelse(type %in% c("Gas", "Electric"), ymax+0.05, ymin-0.05),
                label=scales::percent(percent), color=type), family=font, size=2.5, hjust=0)+
  scale_color_manual(values=rev(met.brewer("Monet", 4)))+
  scale_fill_manual(values=rev(met.brewer("Monet", 4)))+
  facet_wrap(~area_name)+
  coord_equal(clip="off")+
  theme_void()+
  theme(plot.title=element_text(family=font_t, size=18, hjust=0, face="bold", color="#000000"),
  ...
  labs(title="What are people driving?",
       subtitle = "North Carolina's 2022 vehicle registration data highlight vehicle\npreferences of residents in low and high per capita incomes counties, \nsegmented by fuel type percentages. \n\n",
       caption = "\n\n\n#30DayChartChallenge | Day 1| Data: osbm.nc.gov | Design: Ryan Hart | Adapted: Neili Bermudez")

# save plot
ggsave(paste0("day_1_", format(Sys.time(), "%d%m%Y"), ".png"), dpi=320, width=8, height=6)
