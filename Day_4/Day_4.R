### 30DayChartChallenge | May 2023 - Day 4| Historical ###
...
setwd("C:/Users/ncber/OneDrive/Documentos/Practice in R/30daysChallenge/Day_4")

# create plot
df %>%
ggplot(aes(x = Year, y = sold, color = model)) +
  geom_area(aes(fill = model, group = model, alpha = model), position = "identity", color="#FFFFFF", linewidth=0.1)+
  annotate("text", x=2013.5, y=180000, label="Wrangler", color="#000000", family=font_t, size=5, angle=49)+
  annotate("text", x=2015.5, y=115000, label="4Runner", color="#FFFFFF", family=font_t, size=5, angle=38)+
  annotate("segment", x=2018, y=280000, xend=2018, yend=240100, color="#000000", linewidth=0.4, linetype="dotted")+
  annotate("text", x=2017.75, y=280000, label="Wrangler outsold 4Runner\nby 100,338 vehicles in 2018", color="#000000", family=font, size=4, hjust=1, vjust="top")+
  scale_fill_manual(values = c("#5b7314", "#87bcbd"))+
  ...
  theme(plot.title=element_text(family=font_t, size=20, hjust=0, face="bold", color="#000000"),
        ...
        plot.caption = element_text(size=16, family=font, color="#000000", hjust=0.5),
        legend.position = "none",
        axis.text = element_text(family=font, size=10, hjust=0.5, color="#000000", vjust=-2),
        axis.title.y = element_text(family=font, size=10, hjust=0.5, color="#000000"),
  ...
  labs(title = "Wrangler vs 4Runner - U.S. Sales",
       ...
       caption = "\n#30DayChallenge, Day 4 | Data: goodcarbadcar.net | Design: Ryan Hart | Adapted: Neili Bermudez",
       y = "Vehicles Sold\n")       
 
# save plot       
ggsave(paste0("day_4_", format(Sys.time(), "%d%m%Y"), ".png"), dpi=320, width = 1080, height = 800, units = "px")        
#review  